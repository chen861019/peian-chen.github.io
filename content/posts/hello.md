---
title: ""
date: 
draft: false
---


<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<link rel="stylesheet" type='text/css' href="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/devicon.min.css" />

<div class="section-container">
<h1 id="about" class="title" style="margin-bottom:0px;"><i class="fa-solid fa-circle-info"> 關於我</h1></i>
<div class="profile-container">
<div class="profile-pic-wrapper">
    <div class="profile-pic">
        <img src="img/avatar-peian1.jpg" id="avatar" alt="個人照" >
    </div></div>
    <div class="profile-text">
        <h1 style="text-align: center;  line-height:30px; margin-top:30px;margin-bottom:5px; margin-left:-40px;font-size: 22px;">陳沛安<br> peian chen</h1>
        <div class="info-wrapper">
        <ul>
            <li><span class="fa-li"><i class="fa-solid fa-envelope"></i></span><a href="chen861019@gmail.com" class="href">chen861019@gmail.com</a></li>
            <li><span class="fa-li"><i class="fa-brands fa-github"></i></span><a href="https://github.com/chen861019" class="href">github.com/chen861019</a></li>
            <li><span class="fa-li"><i class="fa-solid fa-graduation-cap"></i></span style="font-size:16px;">國⽴陽明交通⼤學 博⼠ 電機⼯程學系</li>
                    </ul>
        <p style="font-size:16px; margin-left:-30px;">具備嵌入式系統、FPGA與數位電路設計以及感測系統平台相關經驗的電機博⼠候選⼈。熟悉微控制器韌體開發、物聯網系統架構以及邊緣⼈⼯智慧（TinyML），並具備完整的軟硬體整合與系統開發能⼒。期望從事嵌入式系統、數位IC/FPGA設計、邊緣AI與智慧感測系統相關之研發⼯程職務。</p>
        </div>
    </div>
</div>
</div>

<div class="section-container"> <h1 id="language" class="title"><i class="fa-solid fa-language"> 語言</h1></i>
    
<div class="language-container">
                <div class="language-item">Chinese</div>
                <div class="language-item">English</div>
                <div class="language-item">Japanese <br>(basic)</div>
            
</div>

<div class="section-container"> <h1 id="skill" class="title"><i class="fas fa-lightbulb"> 專長</h1></i>
    
<div class="skills-container">
    <ul>
        <li>
            <strong class="skill-class"><i class="fa-solid fa-circle"></i> 程式設計：</strong><br>
            
<div class="skill-item">
                <div class="skill-tag">
                    <i class="devicon-cplusplus-plain skill-icon"></i>
                              <div class="skill-info">
                        <span class="skill-name">C/C++</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fab fa-python skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">Python</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="devicon-labview-plain skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">LabVIEW</span>
                        <span class="skill-desc">瑪卡巴卡作</span>
                    </div>
                </div>
            </div>
<div class="skill-item">
                <div class="skill-tag">
                    <i class="devicon-kotlin-plain skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">Kotlin</span>
                        <span class="skill-desc">（Android）瑪卡巴卡作</span>
                    </div>
                </div>
            </div>
        </li>

<li>
            <strong class="skill-class"><i class="fa-solid fa-circle"></i> 嵌入式系統 / AIoT：</strong><br>
            
<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-wifi skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">ESP32</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-microchip skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">nRF52840</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
              <i class="devicon-raspberrypi-plain skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">Raspberry Pi</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-brands fa-bluetooth skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">BLE通訊</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-rss skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">IoT系統架構</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-signal skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">即時資料擷取</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>
<li>
           <strong class="skill-class"><i class="fa-solid fa-circle"></i> 邊緣AI與訊號處理：</strong><br>
            
 <div class="skill-item">
                <div class="skill-tag" style="cursor: pointer;" onclick="location.href='#display'">
                    <i class="fa-solid fa-seedling skill-icon"></i>
                    <div class="skill-info">
                        <span class="skill-name">TinyML</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-wave-square skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">感測資料處理</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-wand-magic-sparkles skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">特徵萃取</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-network-wired skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">機器學習</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
            <div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-code-branch skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">演算法設計</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>

<li>
           <strong class="skill-class"><i class="fa-solid fa-circle"></i> 演算法與資料結構：</strong><br>
            
 <div class="skill-item">
                <div class="skill-tag" style="cursor: pointer;" onclick="location.href='#display'">
                    <i class="fa-solid fa-sitemap skill-icon"></i>
                    <div class="skill-info">
                        <span class="skill-name">資料結構</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-circle-nodes skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">圖論演算法</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>

<li>
           <strong class="skill-class"><i class="fa-solid fa-circle"></i> FPGA / 數位電路設計：</strong><br>
            
 <div class="skill-item">
                <div class="skill-tag" style="cursor: pointer;" onclick="location.href='#display'">
                    <i class="fa-solid fa-file-code skill-icon"></i>
                    <div class="skill-info">
                        <span class="skill-name">Verilog</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-diagram-project skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">RTL設計</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-border-all skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">FPGA開發</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>

<li>
           <strong class="skill-class"><i class="fa-solid fa-circle"></i> 硬體與系統整合：</strong><br>
            
 <div class="skill-item">
                <div class="skill-tag" style="cursor: pointer;" onclick="location.href='#display'">
                    <i class="fa-solid fa-plug skill-icon"></i>
                    <div class="skill-info">
                        <span class="skill-name">感測器介⾯設計</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-bolt skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">機電整合</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-robot skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">⾃動化系統</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>

<li>
           <strong class="skill-class"><i class="fa-solid fa-circle"></i> ⼯程設計：</strong><br>
            
 <div class="skill-item">
                <div class="skill-tag" style="cursor: pointer;" onclick="location.href='#display'">
                    <i class="fa-solid fa-compass-drafting skill-icon"></i>
                    <div class="skill-info">
                        <span class="skill-name">機構與CAD設計</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>

<div class="skill-item">
                <div class="skill-tag">
                    <i class="fa-solid fa-droplet skill-icon"></i> 
                    <div class="skill-info">
                        <span class="skill-name">微流道系統平台設計</span>
                        <span class="skill-desc">瑪卡巴卡</span>
                    </div>
                </div>
            </div>
        </li>
    </ul>
</div></div>
<div class="section-container">
<h1 id="experience" class="title"><i class="fa-solid fa-briefcase"> 經歷</h1></i> 
<div class="custom-section">
<div class="exp-part">   
<div class="exp-card first-card">
        <div class="exp-dot" style="opacity: 0;"></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2024</strong>
        <h4 style="margin: 5px 0;font-size:18px;">研究所 </h4>
        <h4 style="margin: 5px 0; font-size:16px;">
        巴拉巴拉</h4>
        <div class="exp-text">
        <div class="exp-box"><strong style="font-size: 16px; ">美國消費性電子展CES ✕ TTA 臺灣科技新創館</strong> <p style="font-size: 16px;">參展</p></div>
                </div></div>
    </div>

<div class="exp-card">
        <div class="exp-dot" ></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2023</strong>
        <h4 style="margin: 5px 0; font-size:18px;">研究所</h4>
        <h4 style="margin: 5px 0; font-size:16px;">巴拉巴拉</h4>
        <div class="exp-text">
       <div class="exp-box"><strong style="font-size: 16px; ">未來科技獎</strong> <p style="font-size: 16px;">得獎</p></div>
        <div class="exp-box"><strong style="font-size: 16px; padding-right:10%;">FITI創新創業激勵計畫</strong> <p style="font-size: 16px;">入圍</p></div></div></div>
    </div>

<div class="exp-card">
        <div class="exp-dot" ></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2021</strong>
        <h4 style="margin: 5px 0; font-size:18px;">研究所</h4>
        <h4 style="margin: 5px 0; font-size:16px;">巴拉巴拉</h4>
        <div class="exp-text">
        <div class="exp-box"><strong style="font-size: 16px; ">生醫創意構想競賽</strong> <p style="font-size: 16px;">佳作</p></div></div></div>
    </div>

<div class="exp-card">
        <div class="exp-dot" ></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2019</strong>
        <h4 style="margin: 5px 0; font-size:18px;">大學</h4>
        <h4 style="margin: 5px 0; font-size:16px;">巴拉巴拉</h4>
        <div class="exp-text">
        <div class="exp-box"><strong style="font-size: 16px; ">科技生活微電腦應用競賽-AIoT智慧機器人競賽</strong> <p style="font-size: 16px;">第一名</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">科技生活微電腦應用競賽-自走車循跡競速賽</strong> <p style="font-size: 16px;">第二名</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">亞洲智慧型機器人大賽-愚公移山</strong> <p style="font-size: 16px;">第一名</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">科技生活微電腦應用競賽-機器視覺競速</strong> <p style="font-size: 16px;">佳作</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">東京威力TEL科創機器人大賽-鋼鐵擂台全國總決賽</strong> <p style="font-size: 16px;">入圍獎</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">程式設計暨資訊應用競賽-創新實作組-智慧影像辨色系統</strong> <p style="font-size: 16px;">入選獎</p></div></div></div>
    </div>

<div class="exp-card">
        <div class="exp-dot" ></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2018</strong>
        <h4 style="margin: 5px 0; font-size:18px;">大學</h4>
        <h4 style="margin: 5px 0; font-size:16px;">巴拉巴拉</h4>
        <div class="exp-text">
        <div class="exp-box"><strong style="font-size: 16px; ">107年度學生社群成果發表專業揚才-物聯網智慧控制社群</strong> <p style="font-size: 16px;">第一名</p></div>
        </div></div>
    </div>

<div class="exp-card">
        <div class="exp-dot" ></div>
        <div class="exp-content">
        <strong style="font-size:20px;">2017</strong>
        <h4 style="margin: 5px 0; font-size:18px;">大學</h4>
        <h4 style="margin: 5px 0; font-size:16px;">巴拉巴拉</h4>
        <div class="exp-text">
        <div class="exp-box"><strong style="font-size: 16px; ">亞洲智慧型機器人大賽-mBot競走</strong> <p style="font-size: 16px;">佳作</p></div>
        <div class="exp-box"><strong style="font-size: 16px; ">全國機器人互動競賽-自走車循跡競速賽</strong> <p style="font-size: 16px;">佳作</p></div></div></div>
    </div>

</div></div>


<div class="section-container">
<h1 id="research" class="title" ><i class="fa-solid fa-magnifying-glass"> 研究與專案經歷</h1></i>
<div  class="research-section">
        <ul >
            <li><div class="research-box"><strong>微重⼒模擬系統平台
<br>Microgravity Simulation System Platform</strong><br>
<p class="research-text">● 開發雙軸 Random Positioning Machine (RPM)，⽤於微重⼒模擬實驗設計多軸運動控制的嵌入式控制系統
整合感測器、影像模組與即時資料擷取系統</p>
<p class="research-text">● 設計多軸運動控制的嵌入式控制系統</p>
<p class="research-text">● 整合感測器、影像模組與即時資料擷取系統</p>
<div class="technology-item"><span class="technology">技術：嵌入式系統、運動控制、感測系統整合</span></div></div>
</li>
            <li style="padding-top:15px;"><strong>【職場經歷與問題解決：標準化與整合化】</strong><br>
畢業後的兩份工作雖然偏向行政與管理，但我習慣尋找更具效率的作業方式，將邏輯思考應用於實務：<ul style="margin-left:10px;">

<li>● 管理實務（流程標準化）： 在第一份工作期間，我因熟悉業務並具備實務處理經驗獲留任並晉升督導。除了負責人力調度，我進一步將經驗整理為 SOP 作業手冊，這除了輔助教育訓練過程，也讓團隊的行政流程更具穩定性。</li>

<li>● 工具實作（邏輯整合化）： 擔任專任助理時，面對每年累積的大量申請資料，因現有資源無法支援複雜審查，我以 Excel 邏輯公式設計了整合式申請表， 將傳統的「人工核對」改為透過公式進行「邏輯判定」，包含判定時效、核算點數，處理了跨年度重複審查的作業，減少人為核算誤差並優化審查速度。</li></ul>
</li><li><strong>【研究動機與展望：持續探索與深化研究能力】</strong><br> 在接觸資訊領域的過程中，我對計算機圖學能將抽象數據轉化為具體視覺成果的特性深感興趣。這種視覺化產出的過程，讓理論實踐更有實感，也促使我選擇投入此領域作為研究方向。<br>
雖然目前在此專業領域上仍有精進空間，但我期望未來能透過持續的實作與開發，逐步深化技術能力，並累積穩固的研究基礎。
</li>
        </ul>
    </p>
</div></div>

<div class="section-container" style="margin-top: 0px;">
<h1 id="display"  class="title" style="margin-top:10px; "><i class="fa-solid fa-table"> 實作展示 - 整合式申請表</h1></i>


<div class="comparison-container" >
  <div class="comparison-header">
    <div class="comp-card legacy">
    <div style="display:flex; flex-direction: column; align-items: center;">
      <div class="comp-tag">PAST: 傳統紙本</div>
      <div class="comp-content">
        <p><i class="fa-solid fa-x"></i> 每年重複填寫，資料斷裂難追蹤</p>
        <p><i class="fa-solid fa-x"></i> 人工核對舊檔，重複審查耗時</p>
        <p><i class="fa-solid fa-x"></i> 效期手動計算，繁雜且易出錯</p>
      </div>
    </div></div>
    
<div class="comp-vs">VS</div>

<div class="comp-card optimized">
<div style="display:flex; flex-direction: column; align-items: center;">
      <div class="comp-tag primary">NOW: 整合優化</div>
      <div class="comp-content primary-content">
        <p>  <i class="fa-solid fa-v"></i> <strong>歷程資料收錄</strong>：一次填寫，長期存查</p>
        <p>  <i class="fa-solid fa-v"></i> <strong>歷史狀態維護</strong>：僅需審查當年度增量</p>
        <p>  <i class="fa-solid fa-v"></i> <strong>時效規則判定</strong>：自動篩選與點數彙整</p>
      </div>
    </div>
  </div> </div>

  <div class="step-nav">
     </div>
</div>

<section class="excel-container">
<p style="margin-top:30px; margin-bottom:0px; padding-bottom:0px; font-size:14px; padding-left: 1.5em; text-indent: -1.2em; ">※ 本表單僅供功能演示，已移除部分資訊及功能。</p><p style="font-size:14px; margin:0; padding-left: 1.5em; text-indent: -1.2em; ">※ 建議使用電腦操作，如顯示錯誤，請更新頁面。</p><p style="font-size:14px; margin:0; padding-left: 1.5em; text-indent: -1.2em; ">※ 申請表補充：</p><p style="font-size:14px; margin:0; padding-left: 2.8em; text-indent: -1.2em; ">1. 此表設定每筆適用年限為5年，每2點為一基數，其作為核發金額基準。</p><p style="font-size:14px; margin:0; padding-left: 2.8em; text-indent: -1.2em; margin-bottom:30px;">2. 點擊「點此移至空白欄位填寫」可跳至空白欄位做填寫。</p>
<div class="excel-guide-container">
  <div class="step-nav">
    <div class="step-box" id="step1">
      <div class="step-num">01</div>
      <div class="step-info">初始化<br>年限基準設定
      </div><p class="step-text"><strong>● 操作：</strong>頂端輸入「當年度」。</p><p class="step-text"><strong>○ 功能：</strong>作為「點數效期」之判定基準。</p>
      <div class="scan-line"></div>
    </div>
    
<div class="step-arrow">
      <svg viewBox="0 0 24 24"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41z"/></svg>
    </div>

<div class="step-box" id="step2">
      <div class="step-num">02</div>
      <div class="step-info">申請人填寫<br>資料紀錄與引導</div><p class="step-text"><strong>● 操作：</strong>於底色標示處新增資料。</p><p class="step-text"><strong>○ 項目連動：</strong>依項目自動帶入對應點數與備註，輔助申請人填寫。</p><p class="step-text"><strong>○ 歷程存查：</strong>規範數據格式，確保年度資料具備可比對性。</p>
      <div class="scan-line"></div>
    </div>

<div class="step-arrow">
      <svg viewBox="0 0 24 24"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41z"/></svg>
    </div>

<div class="step-box" id="step3">
      <div class="step-num">03</div>
      <div class="step-info">審查填寫<br>效期判定與核定</div><p class="step-text"><strong>● 操作：</strong>審查人員核對並填寫底色標示處。</p><p class="step-text"><strong>○ 時效過濾：</strong>逾期項目自動標示失效不予計點。
</p><p class="step-text"><strong>○ 數據彙整：</strong>自動加總有效點數並轉換為基數。
</p>
      <div class="scan-line"></div>
    </div>
  </div>

  <div class="excel-frame-container" id="excelContainer">
    <div class="excel-loading-overlay" id="excelOverlay">
      <div class="unlock-text">點擊 操作申請表</div>
    </div>
    <div class="excel-display">
    <iframe src="https://1drv.ms/x/c/73b1cde18a4d20c7/IQTQCJOX71ueSbGBbXcylEGmAf-oAclUp9hFzVi38szUVeE?em=2&wdAllowInteractivity=False&AllowTyping=True&wdHideGridlines=True&wdHideHeaders=True&wdDownloadButton=True&wdInConfigurator=True&wdInConfigurator=True" width="100%" 
        height="800px" 
        style="border: 1px solid #ccc; border-radius: 8px;" 
        frameborder="0" 
        scrolling="no"></iframe>
  </div></div>
</div>

